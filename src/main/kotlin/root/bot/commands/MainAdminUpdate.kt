        ...
        MAIN_MENU_ADD -> {
            when (message(upd).text) {
                ...
                text.addMenuSuperAdmin -> {
                    ...
                    user = message(upd).from
                    send.invoke(msgBackMenu(text.msgAddSuperAdmin, text.back).apply { chatId = chatId(upd).toString() })
                }
                text.addMenuMission -> {
        ...
        MAIN_MENU_DELETE -> {
            when (message(upd).text) {
                ...
                text.deleteMenuSuperAdmin -> {
                    ...
                    user = message(upd).from
                    send.invoke(msgBackMenu(text.msgDeleteSuperAdmin, text.back).apply {
                        chatId = chatId(upd).toString()
                    })
                }
            ...
            when (message(upd).text) {
                text.back -> actionBack.invoke()
                    ...
                    text.sendCampaignsTable -> {
                        sendTable.invoke(message(upd).chatId, service.getAllCampaigns(), "")
                    }
                    text.sendSuperAdminTable -> {
                        sendTable.invoke(message(upd).chatId, service.getAllSuperAdmins(), "")
                    }
